#include "UniqueInlineBuffer.h"

UniqueInlineBuffer::UniqueInlineBuffer(const ScalarType& rowType, const ScalarType& valueType): _rowType(rowType), _valueType(valueType)
{
...
Statistic UniqueInlineBuffer::GetStatistic()
{
	Statistic stat;
	stat.total = _count;
	stat.unique = _count;
...
void UniqueInlineBuffer::Apply(const ColumnWrites& writes)
{
...
bool UniqueInlineBuffer::Include(void* rowId, void* value)
{
...
bool UniqueInlineBuffer::Exclude(void* rowId, void* value)
{
	BTree::Path rowpath;
	_rows->GetPath(rowId, rowpath);
	if (!rowpath.Match)
...
bool UniqueInlineBuffer::Exclude(void* rowId)
{
	void* val = GetValue(rowId);
	return Exclude(rowId, val);
}

RangeResult UniqueInlineBuffer::GetRows(const RangeRequest& range)
{
	...
	while (begin != end && !result.limited)
	{
		...
		vrl.push_back(vr);
		result.__set_limited(vrl.size() == size_t(range.limit));

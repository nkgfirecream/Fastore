...
namespace Fastore.Core
{
    ...
    public class PredictionBTree<Key, Value> : IKeyValueTree<Key, Value>
    {
		public PredictionBTree(IComparer<Key> comparer = null, int fanout = 128, int leafSize = 128)
		{
		...
		class Branch : INode
		{
			public Branch(PredictionBTree<Key, Value> tree)
			{
				_tree = tree;
			...
			public Branch(PredictionBTree<Key, Value> tree, INode left, INode right, Key key)
				: this(tree)
			{
				_children[0] = left;
				_children[1] = right;
			...
			private PredictionBTree<Key, Value> _tree;
		...
		class Leaf : INode, IKeyValueLeaf<Key, Value>
		{
			...
			public Leaf(PredictionBTree<Key, Value> parent)
			{
				_tree = parent;

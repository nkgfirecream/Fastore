...
{
	if( random_fd  == -1 ) {
		THROW_ERRNO("could not open random-number generator", random_dev );
	}

	struct stat sb;
	int fOK;

	// create worker's root directory if need be 
	if( (fOK = stat(dirName.c_str(), &sb)) == -1 ) {
		if( (fOK = mkdir(dirName.c_str(), 0700)) != 0 ) {
			THROW_ERRNO("could not find directory name", dirName );
		}
	}
	...
	if( (fOK = stat(dir.c_str(), &sb)) == -1 ) {
		if( (fOK = mkdir(dir.c_str(), 0700)) != 0 ) {
			THROW_ERRNO( "could not create directory name", dir );
		}
	}
	...
	if( -1 == (ssize_t)now) {
		THROW_ERRNO( "could not establish time in", wal );
	} 
	...
	if( (fd = open( filename.c_str(), flags, 0600)) == -1 ) {
		THROW_ERRNO( "could not open WAL file", filename );
	}

	// set the WAL file's size
	off_t offset;
	if( (offset = lseek( fd, (WAL_FILE_SIZE >> 4) - 1, SEEK_SET)) == -1 ) {
		THROW_ERRNO( "could not set size for WAL file", filename );
	}
  
	if( 1 != write(fd, "\0", 1) ) {
		THROW_ERRNO( "write to end of WAL file", filename );
	}
  
	// map in the memory
	static const int wal_prot = (PROT_READ | PROT_WRITE);
	static const int wal_flags = (MAP_SHARED);
	this->wal = static_cast<unsigned char*>( 
											mmap(NULL, WAL_FILE_SIZE, wal_prot, wal_flags, fd, 0) );
	if( wal == MAP_FAILED ) {
		THROW_ERRNO( "could not map WAL file", filename );
	}
...
struct header_t
{
	...
	{
		fingerprint_t fingerprint;
		if( this->fingerprint != compute_stamp(fingerprint) ) {
			Log << log_err << __func__ << ": fingerprint mismatch" << log_endl;
			return false;
		}
		return true;
	}
...
Wal::init( unsigned char *wal )
{
	...
	if( -1 == msync(wal, current - wal, MS_SYNC) ) {
		THROW_ERRNO( "msync(2) failed", __FUNCTION__ );
	} 
}

...
namespace Alphora
{
	namespace Fastore
	{
		namespace Client
		{
			...
			void Transaction::LogColumn::InitializeInstanceFields()
			{
				Includes = std::map<object*, object*>();
				Excludes = std::set<object*>();
			}

			const boost::shared_ptr<Alphora::Fastore::Client::Database> &Transaction::getDatabase() const
			{
				return privateDatabase;
			}

			void Transaction::setDatabase(const boost::shared_ptr<Alphora::Fastore::Client::Database> &value)
			{
			...
			Transaction::Transaction(const boost::shared_ptr<Alphora::Fastore::Client::Database> &database, bool readIsolation, bool writeIsolation)
			{
				...
				_transactionId = boost::make_shared<TransactionID> {Key = 0, Revision = 0};
			...
			std::map<int, ColumnWrites*> Transaction::GatherWrites()
			{
				...
				{
					boost::shared_ptr<ColumnWrites> writes = nullptr;

					// Process Includes
					for (unknown::const_iterator include = entry->Value->Includes.begin(); include != entry->Value->Includes.end(); ++include)
					{
						if (writes == nullptr)
						{
							writes = boost::make_shared<ColumnWrites>();
							writes->Includes = std::vector<Fastore::Include*>();
						}
